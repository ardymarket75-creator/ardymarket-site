...
const SpacedRepetition = {
    ...
    /**
     * v2.1 — Igual que repasosPendientes pero devuelve los IDs TAL CUAL
     * (sin parseInt). Necesario para módulos cuyos IDs no son numéricos,
     * p. ej. Bet Midrash usa 'modeh-ani:1', 'tehilim-113:1'.
     * No se modifica el método original para no romper eris-lab.
     */
    repasosPendientesRaw(progreso, lang) {
        const hoy = this.hoy();
        const pendientes = [];
        const porIdioma = progreso[lang] || {};
        for (const id in porIdioma) {
